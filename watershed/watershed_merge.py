...
def get_volume_info():
    ...
    chunkNum = np.fromfile(gtmp + 'input.metadata', dtype='uint32')[2:5][::-1]
    # chunk sizes
    chunksizes = np.fromfile(gtmp + 'input.chunksizes', dtype='uint32').reshape(-1,3)[:,::-1]
    s = np.fromfile( gtmp + 'input.total_size', dtype='uint32' )
    print "volume size: "+ str(s)
#    width = np.repeat(s.min(),3)
    width = np.fromfile( gtmp + 'input.width', dtype='uint32' )
...
def watershed_merge( ):
    ...
    for x in xrange(0, s[2], width[2]):
        ...
        for y in xrange(0, s[1], width[1]):
            ...
            for z in xrange(0, s[0], width[0]):
                ...
                segfname = gtmp + 'input' + '.chunks/' + str(xind) + '/' + str(yind) + '/' + str(zind) + '/.seg'
    ...
    dendValues = np.fromfile( gtmp + 'input.dend_values', dtype='single' )
    dend = np.fromfile( gtmp + 'input.dend_pairs', dtype = 'uint32' )
