...
if not os.path.exists('../znn/data'):
     os.makedirs('../znn/data')
else:
	#For test
	import shutil
	shutil.rmtree('../znn/data')
	os.makedirs('../znn/data')
...
for c in chunks:
	...
	os.makedirs('../znn/data/{0}'.format(c['filename']))

	#We create a folder to store the chuncked stack as a raw file of double
	os.makedirs('../znn/data/{0}/input'.format(c['filename']))
	...
	chunk.tofile('../znn/data/{0}/input/input'.format(c['filename']))
	sz = numpy.asarray(chunk.shape).astype(numpy.uint32)[::-1]
	sz.tofile('../znn/data/{0}/input/input.size'.format(c['filename']))

	#We also create an empty directoy in where znn will save the output
	os.makedirs('../znn/data/{0}/output'.format(c['filename']))
	...
	os.makedirs('../znn/data/{0}/data_spec'.format(c['filename']))
	znn.stage1Data(c)
	znn.stage2Data(c)
	...
	os.makedirs('../znn/data/{0}/trainning_spec'.format(c['filename']))
	...
	with open('../znn/data/{0}/trainning_spec/run.sh'.format(c['filename']),'w') as runfile:
	...
	#For test
	jobs.write('../znn/data/{0}/trainning_spec/run.sh \n'.format(c['filename']))
