...
function ba2cfg(ba::H5sBigArray)
    config = TCfg()
    config[:prefix] = ba.prefix
    config[:blocksz] = ba.blocksz
    config[:chunksz] = ba.chunksz
    config
end
...
"""
function updateconfig(ba::H5sBigArray)
    fconfig = joinpath(dirname(ba.prefix), "config.json")
    cfg = ba2cfg(ba)
    save(fconfig, cfg)
end
"""
...
function H5sBigArray()
    prefix::AbstractString="/tmp/block_"
    blocksz::Vector = [4096, 4096, 512]
    chunksz::Vector = [256, 256, 32]
    ba = H5sBigArray(prefix, blocksz, chunksz)
    updateconfig(ba)
    return ba
end
