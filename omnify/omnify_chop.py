#!/usr/bin/env python
__doc__ = """
chop the watershed segmentation for omnification
...
def prepare_blocks():
    ...
    blockid = 0
    for bz in xrange(0, s[0], gom_blocksize[0]-gom_overlap[0]):
        for by in xrange(0, s[1], gom_blocksize[1]-gom_overlap[1]):
            for bx in xrange(0, s[2], gom_blocksize[2]-gom_overlap[2]):
                blockid += 1
                ...
                bto = np.minimum(s, bfrom+gom_blocksize)
...
def omnify_chop():
    ...
    ftmp.close()
#    import os
#    os.system('rm -rf {}chunk_*'.format(gom_data_path) )
