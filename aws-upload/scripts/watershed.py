...
for chunk_dir in os.listdir('../data/'):
	...
	z = m.group(1) ; y = m.group(2); x = m.group(3)
...
zabs = 0
for z_znn in range(max_x):
	yabs = 0
	for y_znn in range(max_y):
		xabs = 0
		for x_znn in range(max_z):
			...
			znn_chunk_size = numpy.prod(numpy.fromfile( '../data/{0}/output/stage21.0.size'.format(chunk_dir), dtype='uint32'))
			znn_chunk_0 =  numpy.fromfile( '../data/{0}/output/stage21.0'.format(chunk_dir), dtype='uint32').reshape(znn_chunk_size)
			znn_chunk_1 =  numpy.fromfile( '../data/{0}/output/stage21.1'.format(chunk_dir), dtype='uint32').reshape(znn_chunk_size)
			znn_chunk_2 =  numpy.fromfile( '../data/{0}/output/stage21.2'.format(chunk_dir), dtype='uint32').reshape(znn_chunk_size)
