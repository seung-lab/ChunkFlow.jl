...
public:
    ...
    void update_chunk_data( chunk_type c )
    {
        ...
        {
            mutex::guard g( super_type::mutex_ );
            std::cout << "updated_chunk: " << (*c)  << std::endl;
        }
    ...
    void operator()( bool verbose = true )
    {
        ...
        detail::xxl_queue< edge_type, edge_less< value_type, id_type > > queue2;

        for ( size_type i = 0; i < super_type::size(); ++i )
        {
            chunk_type c = super_type::chunks_.at( i );
            queue2.add_source( c->dendr().filename(), c->dendr().stored_size() );
        }
        ...
        while ( !queue2.empty() )
        {
            if ( zi::get< 2 >( queue2.top() ) < low_threshold )
            {
                break;
            }

            const id_type a1  = zi::get< 0 >( queue2.top() );
            const id_type a2  = zi::get< 1 >( queue2.top() );
            ...
            if ( v1 != v2 )
            {
                dend_vals_out << zi::get< 2 >( queue2.top() );
        ...
        if ( verbose_ )
        {
            std::cout << "created new dendr with len: " << round << std::endl;
        }
