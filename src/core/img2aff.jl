...
function create_config(d)
    ...
    outsz = d[:outsz]
    # configuration string, note that spipe use x,y,z order,
    # and ZNN use z,y,x order
    conf="""
    [parameters]
    ...
    forward_outsz = $(outsz[3]),$(outsz[2]),$(outsz[1])
...
function img2aff( d::Dict{Symbol, Any} )
    ...
    # crop image, affinity map was already cropped in znn
    # for ZNNi, we need to crop affinity map
    if !d[:is_bd_mirror]
        cropsize = (d[:fov]-1) / 2
        # crop image
        img = readimg(d[:fimg])
        ...
        rm(d[:fimg])
        saveimg(d[:fimg], img, "main")
        # crop affinity
        # aff = readaff(outfname)
        # rm(outfname)
        # saveaff(outfname, aff, "main")
    end
    if outfname != d[:faff]
