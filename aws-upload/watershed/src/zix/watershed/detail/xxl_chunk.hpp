...
public:
    ...
    xxl_chunk( std::size_t xpos,
        ...
        : xpos_( xpos ),
          ...
          chunk_( new chunk_type( xw, yw, zw, false ) ),
          conn_ ( new conn_type( xw, yw, zw, prefix_ + "affinity.data", false ) ),
          pos_(),
    ...
    const std::size_t xpos() const
    {
        return xpos_;
    }

    const std::size_t ypos() const
    {
        return ypos_;
    }

    const std::size_t zpos() const
    {
        return zpos_;
    }
    ...
    const std::size_t pos( const std::size_t i ) const
    {
        return pos_[ i ];
    }
    ...
    void save_border_affinities()
    {
        ...
        conn_->xslice( 1, 0, v );
        mmap_file::write( prefix_ + "0affinity", v.begin(), v.end() );

        conn_->yslice( 1, 1, v );
        mmap_file::write( prefix_ + "1affinity", v.begin(), v.end() );

        conn_->zslice( 1, 2, v );
        mmap_file::write( prefix_ + "2affinity", v.begin(), v.end() );
    }
    ...
    {
        mmap_file::write( prefix_ + "seg.data",
                               chunk_->data(),
    ...
    {
        count_ = v.size();
        mmap_file::write( prefix_ + "sizes.data", v.begin(), v.end() );
    }

    void save_dendr( const dendr_type& v )
    {
        own_dendr_size_ = v.size();
        mmap_file::write( prefix_ + "dendr.data", v.begin(), v.end() );
    }
    ...
    void get_sizes( std::vector< sizes_type >& v )
    {
        if ( count_ )
        {
            ...
            v.resize( cnt );
            mmap_file::read_n( prefix_ + "sizes.data", v.begin(), cnt );
        }
    }
    ...
    void get_dendr( dendr_type& v )
    {
        if ( own_dendr_size_ )
        {
            ...
            mmap_file::read_n( prefix_ + "dendr.data", v.begin(), own_dendr_size_ );
        }
    }
    ...
    {
        chunk_->allocate();
        mmap_file::read( prefix_ + "seg.data",
                         chunk_->data(),
                         chunk_->size() * sizeof( id_type ) );
        return chunk_;
    }

    const std::size_t merge_graph_size() const
    {
        return merge_graph_size_;
    }

    const std::size_t merge_pairs_size() const
    {
        return merge_pairs_size_;
    }

    const std::size_t dendr_size() const
    {
